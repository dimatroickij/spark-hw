...
schema = StructType([StructField('idTweet', StringType(), False),
                     StructField('screenName', StringType(), False),
...
retweetsPerMinutes = kafkaStream.map(lambda x: x[1])

countRetweetsPerMinutes = retweetsPerMinutes.map(lambda x: (x, 1)).reduceByKey(lambda x, y: x + y).\
    updateStateByKey(updateCountRetweets)

sortedCountRetweetsPerMinutes = countRetweetsPerMinutes.transform(lambda x: x.sortBy(lambda y: -y[1]))
